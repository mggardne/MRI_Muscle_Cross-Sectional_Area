...
%     command window and written to an MS-Excel spreadsheet. Plots are
%     created of the raw image, threshold histogram, muscles,
%     subcutaneous fat and noncontractile elements. Optionally, the left 
%     and right muscle images may be saved to separate files. 
%
...
%             mthresh.xlsx in the PTOA Subjects\Muscle Scans\MuscleCSA\
...
d = dir(path);  
rdir = 'Muscle CSA';    % Results folder
if ~exist(rdir,'dir')   % Create results directory if it does not exist
  mkdir(rdir);
end 
   
% scans = dir('*.dcm');   % identify all AX FFE scans
% S = numel(scans);       % # of AX FFE scans to read
...
ulbls = [{'','','',''} cellstr(repmat('(cm^2)',50,1))'];     % Units
...
bwml2 = grayconnected(imgbwl,y2,x2,1);  % Get connected muscle
%
bwml = bwml | bwml2;
imgml = imgt;
imgml(~bwml) = 0;
...
bwmr2 = grayconnected(imgbwr,yr2,xr2,1); % Get connected muscle
